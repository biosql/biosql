-- $Id$ 
--
-- THIS FILE IS AUTO-GENERATED FROM biosqldb-mysql.sql. ANY CHANGES YOU
-- MAKE MAY BE LOST UPON RE-GENERATION.
--
-- Having said that, the above is not entirely true. I've replaced a couple
-- of TEXT types with VARCHAR(4000) to achieve better comparability with
-- the Oracle implementation. Based on how I understand the PostgreSQL
-- documentation, this replacement shouldn't have an effect though other
-- than throwing an error when you try to insert a value bigger than 4000
-- chars. (Obviously, this comment was also added ...)
--
-- conventions: 
...
CREATE TABLE reference ( 
	 ...
	 reference_location VARCHAR(4000) NOT NULL , 
	 reference_title VARCHAR(4000) , 
...
CREATE TABLE seqfeature_qualifier_value ( 
	 ...
	 qualifier_value VARCHAR(4000) NOT NULL , 
...
ALTER TABLE remote_seqfeature_name ADD CONSTRAINT FKfeatloc_remotefeat FOREIGN KEY ( seqfeature_location_id ) REFERENCES seqfeature_location ( seqfeature_location_id ) ON DELETE CASCADE ; 
